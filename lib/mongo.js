...
class MongoLib {
    ...
    async create(collection , data) {
        try {
            ...
            console.log(chalk.blue("[CREATE]: ", await result.insertedId))//deberia ir toArray
    ...
    async update(collection, id, data) {
        try {
            const db = await this.connect()
            const result = await db.collection(collection).updateOne({ _id: ObjectId(id)},{ $set: data }, { upsert: true } )//.toArray()
    ...
    async delete(collection, id) {
        try {
            const db = await this.connect()
            const result = await db.collection(collection).deleteOne({ _id: ObjectId(id)} )//.toArray()
            return result
        } catch (error) {
